...
class XHTMLK8Processor:
    ...
    def buildXHTML(self):
        ...
        url_img_index_pattern = re.compile(rb"""[('"]kindle:embed:([0-9|A-V]+)\?mime=image/[^\)]*["')]""", re.IGNORECASE)
        ...
        if self.viewport:
            ...
            viewport_pattern = re.compile(rb"""<meta\s[^>]*name\s*=\s*["'][^"'>]*viewport["'][^>]*>""", re.IGNORECASE)
            for i in range(len(parts)):
                part = parts[i]
